...
# {"file":"scripts/build-and-run.sh","purpose":"Automates dependency checks and desktop builds for Glint3D on Unix-like systems.","exports":[],"depends_on":["cmake","pkg-config"],"notes":["glfw_managed_dropin_supported","assimp_oidn_support"]}
# Human Summary
...
if [ -z "$GLINT_EXE" ]; then
    echo "ERROR: glint executable not found. Please run scripts/build-and-run.sh first." >&2
    exit 1
fi
...
if ! command -v doxygen &> /dev/null; then
    ...
    echo "Then run: scripts/generate-docs.sh"
else
