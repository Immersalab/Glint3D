...
{"file":"tools/qem_simplifier/README.md","purpose":"Documents the QEM tool vertical-slice scaffold and how to build/test it before real QEM implementation.","exports":["build_instructions","scope","testing_flow"],"depends_on":["tools/qem_simplifier/CMakeLists.txt","tools/qem_simplifier/docs/BACKEND_INTERFACE_DISCUSSION.md"],"notes":["external_ui_first","stubbed_qem_core","cli_render_bridge"]}
<!-- Human Summary -->
...
```bat
tools\qem_simplifier\build-and-run.bat debug smoke
tools\qem_simplifier\build-and-run.bat release run
```

Unix-like:

```bash
./tools/qem_simplifier/build-and-run.sh debug smoke
./tools/qem_simplifier/build-and-run.sh release run
```
...
```powershell
cmake -S tools/qem_simplifier -B builds/qem_simplifier/cmake
...
./builds/qem_simplifier/cmake/Release/glint_qem_studio_shell.exe --config tools/qem_simplifier/qem_shell.config
```
...
1. Edit `tools/qem_simplifier/qem_shell.config`:
   - `glint_executable`
   - `mesh_path` (for example `resources/models/cow.obj`)
   - `output_dir` (recommended: `tools/qem_simplifier/output`)
   - `preview_preset_ops` (recommended: `tools/qem_simplifier/output/preview_preset.ops.json`)
...
2. Run the shell with `--config tools/qem_simplifier/qem_shell.config`
3. In the shell, use:
   - `config show`
...
3. Export a preset from Glint:
   ...
   - or console: `export_preview_ops tools/qem_simplifier/output/preview_preset.ops.json`
